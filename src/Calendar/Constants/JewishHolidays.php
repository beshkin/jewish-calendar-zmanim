<?php
namespace Kosherdev\Calendar\Constants;

class JewishHolidays
{
    const EREV_ROSH_HASHANAH = "Erev Rosh Hashanah";
    const ROSH_HASHANAH_I = "Rosh Hashanah I";
    const ROSH_HASHANAH_II = "Rosh Hashanah II";
    const TZOM_GEDALIAH = "Tzom Gedaliah";
    const EREV_YOM_KIPPUR = "Erev Yom Kippur";
    const YOM_KIPPUR = "Yom Kippur";
    const EREV_SUKKOT = "Erev Sukkot";
    const SUKKOT_I = "Sukkot I";
    const SUKKOT_II = "Sukkot II";
    const HOL_HAMOED_SUKKOT = "Hol Hamoed Sukkot";
    const HOSHANA_RABBAH = "Hoshana Rabbah";
    const SHEMINI_AZERET = "Shemini Azeret";
    const SIMCHAT_TORAH = "Simchat Torah";
    const ISRU_HAG = "Isru Chag";
    const SHEMINI_AZERET_SIMCHAT_TORAH = "Shemini Azeret/Simchat Torah";
    const HANUKKAH_I = "Hanukkah I";
    const HANUKKAH_II = "Hanukkah II";
    const HANUKKAH_III = "Hanukkah II";
    const HANUKKAH_IV = "Hanukkah IV";
    const HANUKKAH_V = "Hanukkah V";
    const HANUKKAH_VI = "Hanukkah VI";
    const HANUKKAH_VII = "Hanukkah VII";
    const HANUKKAH_VIII = "Hanukkah VIII";
    const TZOM_TEVET = "Tzom Tevet";
    const TU_BSHEVAT = "Tu B'Shevat";
    const PURIM_KATAN = "Purim Katan";
    const SHUSHAN_PURIM_KATAN = "Shushan Purim Katan";
    const TAANITH_ESTER = "Ta'anith Esther";
    const PURIM = "Purim";
    const SHUSHAN_PURIM = "Shushan Purim";
    const SHABBAT_HAGADOL = "Shabbat Hagadol";
    const EREV_PESACH = "Erev Pesach";
    const PESACH_I = "Pesach I";
    const PESACH_II = "Pesach II";
    const HOL_HAMOED_PESACH = "Hol Hamoed Pesach";
    const PESACH_VII = "Pesach VII";
    const PESACH_VIII = "Pesach VIII";
    const YOM_HASHOAH = "Yom Hashoah";
    const YOM_HAZIKARON = "Yom Hazikaron";
    const YOM_HAATZMAUT = "Yom Ha'Atzmaut";
    const PESACH_SHEINI = "Pesach Sheini";
    const LAG_BOMER = "Lag B'Omer";
    const YOM_YERUSHALAYIM = "Yom Yerushalayim";
    const EREV_SHAVUOT = "Erev Shavuot";
    const SHAVUOT_I = "Shavuot I";
    const SHAVUOT_II = "Shavuot II";
    const TZOM_TAMMUZ = "Tzom Tammuz";
    const TISHA_BAV = "Tisha B'Av";
    const TU_BAV = "Tu B'Av";

}
